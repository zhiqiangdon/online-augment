...
def train_epoch_two_bns(trainloader, model, epoch, config):
    ...
    for i, (input_list, target) in enumerate(trainloader):
        ...
        loss_adv = -loss_aug * model.args.adv_weight_stn
        ...
        # input_aug, target_aug = model.aug_net(noise, input, target)
        # output_aug = model.target_net(input_aug.detach(), 'stn')
        # loss_aug = model.criterion(output_aug, target_aug)

        for n, p in model.target_net.named_parameters():
            if p.grad is not None:
                if 'bn' not in n or 'stn' in n:
