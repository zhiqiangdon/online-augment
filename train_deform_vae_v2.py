...
def reverse_and_scale_grad(scale):
    def hook(grad):
        return grad * (-scale)
    return hook
...
def train_epoch_two_bns(trainloader, model, epoch, config):
    ...
    for i, (input_list, target) in enumerate(trainloader):
        ...
        print('forward without grad reverse hook...')
        input_aug, div_loss, smooth_loss = model.aug_net(input, require_loss=True)
        output_aug = model.target_net(input_aug, 'deform')
        loss_aug = model.criterion(output_aug, target)
        loss_aug.backward()
        grad_saved = {}
        for n, p in model.aug_net.named_parameters():
            grad_saved[n] = p

        model.aug_net.zero_grad()
        model.target_net.zero_grad()
        print('forward with grad reverse hook...')
        input_aug, div_loss, smooth_loss = model.aug_net(input, require_loss=True)
        input_aug.register_hook(lambda grad: grad * (-model.args.adv_weight_deform))
        output_aug = model.target_net(input_aug, 'deform')
        loss_aug = model.criterion(output_aug, target)
        for n, p in model.aug_net.named_parameters():
            print('n diff: {}'.format(torch.dist(p, grad_saved[n])))
        exit()
        # loss_adv = -loss_aug *
