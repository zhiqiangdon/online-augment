...
from .shakeshake import ShortcutMultiBN
from ..custom_layers import MultiBatchNorm

class ShakeBlock(nn.Module):

    def __init__(self, in_ch, out_ch, stride=1, bn_types=None):
        super(ShakeBlock, self).__init__()
        self.bn_types = bn_types
        self.equal_io = in_ch == out_ch
        self.shortcut = self.equal_io and None or ShortcutMultiBN(in_ch, out_ch, stride=stride,
                                                                  bn_types=bn_types)
    ...
    def _make_branch(self, in_ch, out_ch, stride=1):
        return nn.Sequential(
            ...
            MultiBatchNorm('2d', self.bn_types, out_ch),
            nn.ReLU(inplace=False),
...
class ShakeResNetMultiBN(nn.Module):

    def __init__(self, depth, w_base, label, bn_types):
        super(ShakeResNetMultiBN, self).__init__()
        self.bn_types = bn_types
        n_units = (depth - 2) / 6
    ...
    def _make_layer(self, n_units, in_ch, out_ch, stride=1):
        ...
        for i in range(int(n_units)):
            layers.append(ShakeBlock(in_ch, out_ch, stride=stride, bn_types=self.bn_types))
