...
class ShortcutMultiBN(nn.Module):

    def __init__(self, in_ch, out_ch, stride, bn_types=None):
        super(ShortcutMultiBN, self).__init__()
        self.stride = stride
        self.conv1 = nn.Conv2d(in_ch, out_ch // 2, 1, stride=1, padding=0, bias=False)
        self.conv2 = nn.Conv2d(in_ch, out_ch // 2, 1, stride=1, padding=0, bias=False)
        # self.bn = nn.BatchNorm2d(out_ch)
        self.bn = MultiBatchNorm('2d', bn_types, out_ch)
