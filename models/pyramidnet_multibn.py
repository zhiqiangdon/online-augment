...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, p_shakedrop=1.0, bn_types=None):
        super(BasicBlock, self).__init__()
        # self.bn1 = nn.BatchNorm2d(inplanes)
        self.bn1 = MultiBatchNorm('2d', bn_types, inplanes)
        self.conv1 = conv3x3(inplanes, planes, stride)
        # self.bn2 = nn.BatchNorm2d(planes)
        self.bn2 = MultiBatchNorm('2d', bn_types, planes)
        self.conv2 = conv3x3(planes, planes)
        # self.bn3 = nn.BatchNorm2d(planes)
        self.bn3 = MultiBatchNorm('2d', bn_types, planes)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, p_shakedrop=1.0, bn_types=None):
        ...
        self.bn3 = MultiBatchNorm('2d', bn_types, (planes * 1))
        self.conv3 = nn.Conv2d((planes * 1), planes * Bottleneck.outchannel_ratio, kernel_size=1, bias=False)
        # self.bn4 = nn.BatchNorm2d(planes * Bottleneck.outchannel_ratio)
        self.bn4 = MultiBatchNorm('2d', bn_types, planes * Bottleneck.outchannel_ratio)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
        self.shake_drop = ShakeDrop(p_shakedrop)
...
class PyramidNetMultiBN(nn.Module):

    def __init__(self, dataset, depth, alpha, num_classes, bottleneck=True, bn_types=None):
        super(PyramidNetMultiBN, self).__init__()
        ...
        if self.dataset.startswith('cifar'):
            ...
            self.bn1 = MultiBatchNorm('2d', bn_types, self.input_featuremap_dim)
            ...
            self.bn_final = MultiBatchNorm('2d', bn_types, self.final_featuremap_dim)
    ...
    def pyramidal_make_layer(self, block, block_depth, stride=1):
        ...
        layers.append(block(self.input_featuremap_dim, int(round(self.featuremap_dim)), stride, downsample, p_shakedrop=self.ps_shakedrop.pop(0), bn_types=self.bn_types))
        for i in range(1, block_depth):
            temp_featuremap_dim = self.featuremap_dim + self.addrate
            layers.append(
                block(int(round(self.featuremap_dim)) * block.outchannel_ratio, int(round(temp_featuremap_dim)), 1, p_shakedrop=self.ps_shakedrop.pop(0), bn_types=self.bn_types))
